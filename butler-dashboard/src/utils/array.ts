 ...
 * @returns
 */
export const ArrayToObject = <T>(array: Array<T>, property: string): Record<string, T> => array.reduce((acc, obj) => {
  if (!IsObject(obj)) {
    throw new Error(`Not an object, found: ${typeof obj}`);
  }
  const key = obj[property];
  if (!key) {
    throw new Error(`Invalid key: ${key} -- ${key} is missing in object: ${JSON.stringify(obj)}`);
  }
  return { ...acc, [key]: { ...obj } };
}, {});
