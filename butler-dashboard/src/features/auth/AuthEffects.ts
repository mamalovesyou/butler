...
import { addAuthorization, GoogleRpcStatus, V1AuthenticatedUser} from '../../api';
import { AxiosResponse } from 'axios';
import { persistor } from '../index';
import { push } from "redux-first-history";
import { DASHBOARD_ROOT_PATH, LOGIN_ROUTE_PATH, ONBOARDING_ROOT_PATH } from '../../routes';
import { Api } from '../configureEffects';


// Called when a user try to login
export function* onLoginRequest() {
    yield takeEvery(ActionTypes.LOGIN_REQUEST, function* ({ payload }: ActionTypes.ILoginRequest) {
        try {
...
export function* onRefreshTokenRequest() {
    yield takeEvery(ActionTypes.REFRESH_TOKEN_REQUEST, function* ({ payload }: ActionTypes.IRefreshRequest) {
        try {
            const response: AxiosResponse<V1AuthenticatedUser> = yield Api.v1.authServiceRefreshToken(payload)
...
export function* onSignUpRequest() {
    yield takeEvery(ActionTypes.SIGNUP_REQUEST, function* ({ payload }: ActionTypes.ISignupRequest) {
        try {
            const response: AxiosResponse<V1AuthenticatedUser> = yield Api.v1.authServiceSignUp(payload)
