...
func NewCatalog(cfg *config.SourcesConfig, airbyteURL string) *Catalog {
	// Init data-sources
	googleSource := NewGoogleAdWordsSource(cfg.Google, cfg.RedirectURL)
	linkedinSource := NewLinkedinAdsSource(cfg.Linkedin, cfg.RedirectURL)
...
func (catalog *Catalog) ExchangeOAuthCode(ctx context.Context, airbyteSOurceID, code string) (*oauth2.Token, error) {
	...
	if source.AuthScheme() != OAUTH2 {
		return nil, errors.New(fmt.Sprintf("This source dosen't support oauth 2 auth: %s", source.Name()))
	}

	token, err := source.(OAuthSource).ExchangeCode(ctx, code)
	if err != nil {
		logger.Error(ctx, "Unable to exchange oauth code", zap.Error(err), zap.String("provider", source.Name()))
