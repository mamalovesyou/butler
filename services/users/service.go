...
type UsersService struct {
	...
	DestinationCatalog *destinations.DestinationCatalog

	api_users.UnimplementedUsersServiceServer
}

// NewUsersService initialize with predefined configuration
func NewUsersService(config *ServiceConfig, db *gorm.DB, rdb *redis.Client, catalog *destinations.DestinationCatalog) *UsersService {
	jwtManager := jwt.NewJWTManager(config.JWTSecret)
	return &UsersService{
		WorkspaceUsecase:  workspace.NewWorkspaceUsecase(db, config.SendgridAPIKey, config.WebappBaseURL, config.Airbyte.AirbyteServerURL, catalog),
