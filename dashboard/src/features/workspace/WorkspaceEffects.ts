...
import { GoogleRpcStatus, V1OrganizationListResponse, V1OrganizationResponse, V1AuthenticatedUser, V1WorkspaceResponse} from '../../api';
...
export function* onCreateOrganizationRequest() {
    yield takeEvery(ActionTypes.CREATE_ORGANIZATION_REQUEST, function* ({ payload }: ActionTypes.ICreateOrganizationRequest) {
        try {
            const response: AxiosResponse<V1OrganizationResponse> = yield Api.v1.workspaceServiceCreateOrganization(payload)
...
export function* onListWorkspacesRequest() {
    yield takeEvery(ActionTypes.LIST_ORGANIZATIONS_REQUEST, function* () {
        try {
            const response: AxiosResponse<V1OrganizationListResponse> = yield Api.v1.workspaceServiceListOrganizations()
...
export function* onCreateWorkspaceRequest() {
    yield takeEvery(ActionTypes.CREATE_WORKSPACE_REQUEST, function* ({ payload }: ActionTypes.ICreateWorkspaceRequest) {
        try {
            const response: AxiosResponse<V1WorkspaceResponse> = yield Api.v1.workspaceServiceCreateWorkspace(payload)
...
export function* onAddWorkspaceMembersRequest() {
    yield takeEvery(ActionTypes.ADD_WORKSPACE_MEMBER_REQUEST, function* ({ payload }: ActionTypes.IAddWorkspaceMembersRequest) {
        try {
            const response: AxiosResponse<V1AuthenticatedUser> = yield Api.v1.authServiceSignIn(payload)
